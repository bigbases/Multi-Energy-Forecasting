...
for num in range(200,205) :
    ...
    for epoch in range(n_epochs) :
        ...
        for batch,(features, label) in train_ds.enumerate() :
            corr_list = np.zeros((batch_size,4))
...
#         # For valid set
    ...
    result_df.to_pickle('./bahda_corr_divide_lstm1_{}/result_{}.csv'.format(ENERGY, num))
    loss_pred_df = [i.numpy() for i in loss_pred_df]
    loss_corr_df = [i.numpy() for i in loss_corr_df]
    pd.DataFrame(loss_pred_df).to_pickle('./bahda_corr_divide_lstm1_{}/loss_pred_{}.csv'.format(ENERGY, num))
    pd.DataFrame(loss_corr_df).to_pickle('./bahda_corr_divide_lstm1_{}/loss_corr_{}.csv'.format(ENERGY, num))
    loss_test = [i.numpy() for i in loss_test]
    pd.DataFrame(loss_test).to_pickle('./bahda_corr_divide_lstm1_{}/loss_test_{}.csv'.format(ENERGY, num))
    pd.DataFrame(weight.numpy()).to_pickle('./bahda_corr_divide_lstm1_{}/weight_{}.csv'.format(ENERGY, num))
    pd.DataFrame(corr_list).to_pickle('./bahda_corr_divide_lstm1_{}/corrlist_{}.csv'.format(ENERGY, num))
    # val_loss = [i for i in val_loss]
    pd.DataFrame(val_loss).to_pickle('./bahda_corr_divide_lstm1_{}/loss_val_{}.csv'.format(ENERGY, num))
