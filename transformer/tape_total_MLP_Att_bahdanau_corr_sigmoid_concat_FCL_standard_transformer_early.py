...
for num in range(105,110) :
    ...
    # optimizer_pred = keras.optimizers.Adam(learning_rate=0.0001)
    # optimizer_corr = keras.optimizers.Adam(learning_rate=0.0001)
    learning_rate = CustomSchedule(d_model=32)
    optimizer_pred = tf.keras.optimizers.Adam(learning_rate, beta_1=0.9, beta_2=0.98, epsilon=1e-9)
    optimizer_corr = tf.keras.optimizers.Adam(learning_rate, beta_1=0.9, beta_2=0.98, epsilon=1e-9)
    ...
    for epoch in range(n_epochs) :
        ...
        for batch,(features, label) in valid_ds.enumerate() :
            y_pred, _ = model(features[:,:,0],features[:,:,1],features[:,:,2],features[:,:,3],features[:,:,4], 
                       label, training=False)
            # val_metric+=metric(label, y_pred)
            # error_mae = mean_squared_error(label, y_pred)
