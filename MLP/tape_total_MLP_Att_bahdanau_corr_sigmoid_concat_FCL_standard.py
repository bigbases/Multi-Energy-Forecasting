...
for num in range(100,105) :
    ...
    class LinearModel(keras.layers.Layer) :
        def __init__(self, name='linear'):
            super(LinearModel, self).__init__(name=name)
            self.hidden = keras.layers.Dense(1000, kernel_initializer= 'he_normal', activation = 'relu')
    ...
    class Encoder(keras.layers.Layer) :
        def __init__(self, name="encoder"):
            ...
            self.linear_1 = LinearModel()
            self.linear_2 = LinearModel()
            self.dropout_3 = keras.layers.Dropout(0.3)
            self.linear_3 = LinearModel()
            self.dropout_4 = keras.layers.Dropout(0.3)
            self.linear_4 = LinearModel()
